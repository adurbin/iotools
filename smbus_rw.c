...
parse_io_width(const char *arg, struct smbus_op_params *params,
	...
	case SMBUS_QUICK:
		...
		if (ismaxortrailingjunk(arg, end, ldata)) return -1;
		if (ldata != 0 && ldata != 1) {
	...
	case SMBUS_SIZE_32:
		ldata = strtoul(arg, &end, 0);
		if (istrailingjunk(arg, end)) return -1;
...
smbus_read_op(struct smbus_op_params *params, const struct smbus_op *op)
{
	...
	print_read_data(op->size, read_block_size, &params->data);
	return 0;
}
		      ...
		      const struct smbus_op *op)
{
	...
	if (ioctl(params->fd, I2C_SLAVE_FORCE, (void *)(intptr_t)params->address) <
	    0) {
		fprintf(stderr, "can't set address 0x%02X, %s\n", params->address,
			strerror(errno));
		return -1;
	...
	result =
		i2c_smbus_process_call(params->fd, params->reg, params->data.fixed.u16);
	if (result < 0) {
		fprintf(stderr, "SMBUS Process call failed: %s.\n", strerror(errno));
		return -1;
	}

	print_read_data(SMBUS_SIZE_16, 0, &params->data);
	return 0;
}
			    ...
			    const struct smbus_op *op)
{
	...
	read_block_size = i2c_smbus_block_process_call(
		params->fd, params->reg, params->len, params->data.array);
	...
	print_read_data(SMBUS_SIZE_BLOCK, read_block_size, &params->data);
	return 0;
}
